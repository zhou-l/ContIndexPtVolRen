...
%     fileName = sprintf('%s_Oct_eigVolReconst.nrrd', name);
    fileName = sprintf('%s_majEig_gold%i_1.00.nrrd', name, i);
    nameOnlyList{i} = name;
    majEigFileList{i} = fileName;
end

secEigFileList = fileList;
for i = 1:length(secEigFileList)
    [filepath, name, ext] = fileparts(fileList{i});
%     fileName = sprintf('%s_Oct_eigVolReconst.nrrd', name);
    fileName = sprintf('%s_secEig_gold%i_1.00.nrrd', name, i);
    nameOnlyList{i} = name;
    secEigFileList{i} = fileName;
end
...
[secEVlist, mVols, secEvolInfoList] = loadVolumes(secEigFileList);
...
%     mu(:,i) = (reshape(Vlist{i},numVoxels,1) - rangeMu(i,1)) ./ range;
    ...
    secEig(:,i) = reshape(secEVlist{i},numVoxels,1);
end
